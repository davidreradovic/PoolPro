...
import com.example.backend.dto.CreateTaskCommentRequest;
...
public class TaskCommentController {
    ...
    public Object createComment(
        ...
        saved = taskCommentRepository.findById(saved.getIdTaskComment()).orElse(saved);
        ...
        if (saved.getReply() != null) {
            response.put("replyId", saved.getReply().getIdTaskComment());
        }
    ...
    public Object getCommentsByTask(@PathVariable Integer taskId) {
        return taskCommentRepository.findByTask_IdTask(taskId)
                .stream()
                .map(comment -> {
                    Map<String, Object> response = new HashMap<>();
                    response.put("idTaskComment", comment.getIdTaskComment());
                    ...
                    if (comment.getReply() != null) {
                        response.put("replyId", comment.getReply().getIdTaskComment());
                    }
